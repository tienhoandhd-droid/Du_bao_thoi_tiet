...
**Groq `llama-3.3-70b-versatile`** (ai_languageModel index 1) bên cạnh Gemini.
Gemini fail/hết quota → tự chuyển Groq → framing vẫn ra claim. **Verify:** MoA
execution `1587517` SUCCESS, verdict `supported` 0.87, ghi `claim_verdicts`.

### 6e. Dashboard hàng đợi cờ — ĐÃ LÀM + build PASS
`app/src/features/validation/FlagQueuePanel.tsx` + wire vào `App.tsx` (tab
"🚩 Hàng đợi cờ AL"): đọc view `scan_flags_pending`, hiện document/trang/verdict/
