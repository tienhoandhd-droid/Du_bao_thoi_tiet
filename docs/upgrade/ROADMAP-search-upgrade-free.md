...
### GIAI ĐOẠN 4.5 — CRAVE Claim Verification (kiểm chứng nhận định 2 chiều) — **lõi phương pháp**

> CRAVE = *Conflicting Reasoning Approach for explainable claim VErification* (bài 2025): khử mơ hồ +
> truy xuất bằng chứng → lập luận hai hướng ủng hộ/phản bác → mô hình nhỏ phán đoán cuối. Đây là
> "xương sống" cho tên hệ thống: không chỉ **tìm tài liệu** mà **kiểm chứng một khẳng định GMP từ nhiều
> phía**. Tận dụng đúng chiến lược §0 (vòng lặp đa model free) và nhu cầu bằng chứng 2 chiều của thẩm định.

**Pipeline C-R-A-V-E (map vào hệ thống đã có):**

| Bước | Làm gì | Dùng lại / Thêm mới |
|---|---|---|
| **C — Claim** | Câu hỏi VI → mệnh đề kiểm chứng được + tách facet PCC-GMP (Đối tượng/Hoạt động/Bối cảnh/Ngưỡng) | *Mới* (LLM free-tier) |
| **R — Retrieve** | `hybrid_search_v4` (RRF) cho cả mệnh đề **và** phủ định/biến thể của nó | ✅ Đã có |
| **A — Ambiguity** | Khử mơ hồ thuật ngữ song ngữ (*oil aerosol vs oil vapor*, *sampling time*, *LOD/LOQ*) | ✅ `glossary_terms` VI/EN |
| **V — Verify 2 chiều** | **Support-agent** (free #1) gom bằng chứng ủng hộ · **Refute-agent** (free #2, provider KHÁC) gom phản bác/giới hạn | *Mới* — stance-based |
| **E — Explain** | **Judge nhỏ** cân 2 phía → verdict + bảng bằng chứng + độ tin cậy + trích dẫn, **trả lời tiếng Việt** | Nâng cấp faithfulness-check |

**Verdict taxonomy GMP (Judge trả về, không chỉ Đúng/Sai):**

| Verdict | Ý nghĩa |
|---|---|
| `supported` — Được hỗ trợ | Nhiều nguồn có thẩm quyền xác nhận, không mâu thuẫn |
| `conditional` — Có điều kiện | Đúng trong phạm vi/điều kiện cụ thể (nêu rõ) |
| `conflicting` — Mâu thuẫn giữa nguồn | Tiêu chuẩn/phiên bản không thống nhất → con người quyết |
| `outdated` — Đã lỗi thời/bị thay thế | Nguồn cũ bị bản mới thay (gắn `version_status`) |
| `insufficient` — Không đủ bằng chứng | Corpus không đủ → **từ chối trả lời** (no-source refusal ≥90%) |

**Verify 2 chiều bằng free-tier (đúng thang leo thang §0):**
```
Mệnh đề (C) + bằng chứng (R,A)
  ├─ SUPPORT-agent (free #1, vd Gemini)  → trích đoạn ủng hộ + độ mạnh
  ├─ REFUTE-agent  (free #2, vd Groq)    → trích đoạn phản bác/giới hạn/ngoại lệ
  │     (2 provider KHÁC nhau = độc lập thật, không phải 1 model tự phản biện)
  └─ JUDGE nhỏ (free #3) → cân 2 phía → VERDICT + confidence
        └─ GMP tới hạn / bất đồng mạnh → escalate OpenAI → vẫn không chắc → human sign-off (DRAFT)
  └─ ghi mọi lượt + stance vào audit append-only
```

**Xử lý VI→EN→VI:** Claim lưu song ngữ (VI gốc + EN canonical qua glossary); retrieval chạy EN canonical;
**verdict + giải thích tiếng Việt**, nhưng **trích đoạn bằng chứng giữ nguyên EN gốc** + dịch VI đánh dấu
"không chính thức — Xem bản gốc". Không để bản dịch đổi nghĩa quy định.

**Router (không chạy CRAVE cho mọi câu):** câu tra cứu thường → TieredAnswer (nhanh/rẻ); câu **kiểm chứng
nhận định** ("có đúng là…", "có/không", "so sánh", "được phép không") → đường CRAVE đầy đủ.

**Schema thêm (M2):** `claims` (VI+EN, facet PCC) · cột `stance`/`strength` trên `ai_query_sources`
(`support|refute|limiting|neutral`) · `claim_verdicts` (verdict, confidence, rationale_vi, model, escalate,
link `prompt_versions`). Kết hợp PRISMA-funnel → bằng chứng 2 chiều truy vết được cho thanh tra.

| # | Việc | Ai làm | DoD / GMP gate |
|---|---|---|---|
|4.5.1| Migration `claims`/`stance`/`claim_verdicts` (+down, RLS, append-only) | 🟢 CLI | RLS pass, eval baseline không gãy |
|4.5.2| Prompt bộ ba Support/Refute/Judge + verdict taxonomy → `prompt_versions` | 🟢🟡 | Output VI, trích dẫn EN gốc |
|4.5.3| n8n WF CRAVE orchestrator (router → C/R/A → V 2 chiều → E judge → audit) | 🟡 | Free-tier, không hard-code; DRAFT + citation bắt buộc |
|4.5.4| UI verdict badge + bảng Ủng hộ/Phản bác + confidence + "Xem bản gốc" | 🟢 | Deploy Pages |
|4.5.5| Golden claims (supported/conflicting/insufficient) + ngưỡng verdict | 🟢 | faithfulness ≥0,90; cấm sửa số |
...
**Tuần 3–4:** GĐ3–4 (rerank, HyDE, routing, agentic, faithfulness) → **GĐ4.5 CRAVE** (kiểm chứng nhận định 2 chiều — lõi phương pháp cho câu hỏi thẩm định GMP).
