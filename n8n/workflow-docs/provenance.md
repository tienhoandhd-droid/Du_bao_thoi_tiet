...
- Literal Supabase apikey/JWT-like và Tavily key được thay bằng placeholder; không lưu hash của secret để tránh tạo thêm material có thể dùng đối chiếu.
- MCP loại bỏ `credentials`; binding được phục hồi theo source repo/loại node và inventory credential đã phê duyệt. Mỗi binding ghi evidence riêng; chưa được gọi là “live-verified”.

## Ranh giới

Không truy vấn, đọc chi tiết hay thay đổi BMS-GMP, VMP, QMSTeam hoặc GMP Kiểm Tạp. Không update/execute/import/publish/unpublish/archive n8n; không ghi Supabase; không push/PR.
